...
import {
  BarChart3, Key, Cog, CheckCircle, Zap, PieChart, Dice5,
  RefreshCw, Trash2, Download, Plus, LayoutGrid, Clock, Laptop, Server, Atom, Cpu, Loader2,
  MessageSquare, Paperclip, ExternalLink, Eye, CheckCircle2
} from 'lucide-react';
...
type TabType = 'overview' | 'jobs' | 'feedback' | 'admin';
...
export function DashboardContent() {
  ...
  const handleUpdateFeedbackStatus = async (feedback: FeedbackData, newStatus: FeedbackData['status']) => {
    try {
      await updateFeedbackStatus(feedback.feedbackId, feedback.createdAt, newStatus);
      setFeedbackList(feedbackList.map(f =>
        f.feedbackId === feedback.feedbackId ? { ...f, status: newStatus } : f
      ));
  ...
  const handleDeleteFeedback = async () => {
    ...
    try {
      await deleteFeedbackApi(feedbackToDelete.feedbackId, feedbackToDelete.createdAt);
  ...
  return (
    <div>
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg bg-gray-700/50 text-gray-300 hover:bg-gray-700 transition-all disabled:opacity-50 flex items-center"
              >
      ...
      {activeTab === 'feedback' && (
        <div className="animate-fade-in-up">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold flex items-center">
              <MessageSquare className="w-5 h-5 text-[#14b8a6] mr-2" />
          ...
          {isLoadingFeedback ? (
            ...
            <div className="glass-effect rounded-xl p-12 text-center">
              ...
              <p className="text-gray-400">User feedback will appear here when submitted.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {feedbackList.map((feedback) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      <span className={cn(
                        ...
                        feedback.category === 'bug' && 'bg-red-500/20 text-red-400',
                        feedback.category === 'feature' && 'bg-blue-500/20 text-blue-400',
                        feedback.category === 'question' && 'bg-yellow-500/20 text-yellow-400',
                        feedback.category === 'other' && 'bg-gray-500/20 text-gray-400'
                      )}>
                        {feedback.category}
                      </span>
                      <span className={cn(
                        'px-2.5 py-1 rounded-full text-xs font-medium capitalize',
                        feedback.status === 'pending' && 'bg-yellow-500/20 text-yellow-400',
                    ...
                    <div className="flex items-center space-x-2">
                      {feedback.status !== 'reviewed' && (
                        <button
                          onClick={() => handleUpdateFeedbackStatus(feedback, 'reviewed')}
                      ...
                      {feedback.status !== 'resolved' && (
                        <button
                          onClick={() => handleUpdateFeedbackStatus(feedback, 'resolved')}
                          className="p-1.5 rounded-lg hover:bg-green-500/20 text-gray-400 hover:text-green-400 transition-all"
                  ...
                  {/* Message */}
                  <p className="text-gray-300 mb-3 whitespace-pre-wrap">{feedback.message}</p>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {feedback.files.map((file, index) => (
                        <button
                          ...
                          className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-gray-700/50 hover:bg-gray-700 text-sm text-gray-300 transition-all"
                        >
      ...
      <ConfirmModal
        ...
        confirmText={isDeletingFeedback ? "Deleting..." : "Delete"}
      />
    </div>
  );
}
